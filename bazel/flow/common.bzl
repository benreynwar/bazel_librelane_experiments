...
def create_librelane_config(src_info):
    """Create a config dict for librelane from LibrelaneInfo."""
    pdk = src_info.pdk_info
    ...
    config = {
        ...
        "VERILOG_FILES": get_verilog_paths(src_info),
    ...
    # Add macros config
    macros = get_macros_config(src_info)
    if macros:
...
def get_input_files(src_info):
    """Get all input files from a LibrelaneInfo provider.
    ...
    # Add PDK files
    inputs.extend(get_pdk_files(src_info.pdk_info))

    # Add verilog files if present
    if src_info.verilog_files:
        inputs.extend(src_info.verilog_files.to_list())

    # Add state_out from previous step (for metrics)
    if src_info.state_out:
        inputs.append(src_info.state_out)
...
def get_verilog_paths(src_info):
    """Get verilog file paths - either input RTL or synthesized netlist."""
    if src_info.nl:
        return [src_info.nl.path]
    elif src_info.verilog_files:
        return [f.path for f in src_info.verilog_files.to_list()]
    else:
        return []

def get_macros_config(src_info):
    """Build MACROS config dict from MacroInfo list."""
    if not src_info.macros:
        return None
...
def librelane_output_path(filename):
...
def run_librelane_step(
    ...
    Args:
        ...
        src_info: LibrelaneInfo from previous step
    ...
    # Generate state_in from src_info (has correct Bazel paths, no metrics yet)
    state_content = json.encode(serialize_state(src_info))
    ...
    if src_info.state_out:
        prev_state_out = src_info.state_out.path
    ...
    ctx.actions.run_shell(
        ...
        command = """
            ...
            HOME="{design_dir}" librelane.steps run \\
                ...
                -c {config_file} \\
                -i "{state_in}" \\
                -o "{design_dir}/runs/bazel"
            {copy_commands}
        """.format(
            merge_metrics_cmd = merge_metrics_cmd,
            pdk = src_info.pdk_info.name,
            scl = src_info.pdk_info.scl,
...
def single_step_impl(ctx, step_id, extra_config = {}, extra_inputs = []):
    ...
    steps that take def/odb as input and produce updated def/odb as output.
    ...
    # Build new LibrelaneInfo with updated def/odb, preserving other fields
    return [
...
FLOW_ATTRS = {
    "src": attr.label(
        doc = "Previous flow step providing LibrelaneInfo",
        mandatory = True,
        providers = [LibrelaneInfo],
    ),
}

# Attributes for entry-point rules (synthesis)
ENTRY_ATTRS = {
